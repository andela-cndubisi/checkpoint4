package checkpoint.andela.com.productivitytracker.db;
...
public class ProductivityDBHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<LocationItem> getDateWithCount(){
        ...
        for (int i =0; i< cursor.getCount(); i++){
             if (cursor.moveToNext()) {
                 ...
                 LocationItem history = new LocationItem(cursor.getString(dateIdx),cursor.getInt(countIdx),0);
                list.add(history);
            }
        }
        cursor.close();
    ...
    public ArrayList<LocationItem> getSumLocationWithInterval(){
        final String newColumnName = "count";
        final String COLUMN_SELECTION = "sum("+LocationEntry.COLUMN_INTERVAL+")";
        final String newColumn = "(select count("+LocationEntry.COLUMN_INTERVAL+") " +
        ...
        Cursor cursor = db.rawQuery(
                "select " + COLUMN_SELECTION + ", " +
                        LocationEntry.COLUMN_CITY_NAME + "," + newColumn +"," + LocationEntry.COLUMN_INTERVAL +"," + LocationEntry.COLUMN_INTERVAL +
                        " as " + newColumnName +
        ...
        for (int i =0; i< cursor.getCount(); i++){
            if (cursor.moveToNext()) {
                ...
                LocationItem history = new LocationItem(cursor.getString(cityIdx),cursor.getInt(countIdx),cursor.getLong(sumIdx));
                list.add(history);
            }
        }
        cursor.close();
